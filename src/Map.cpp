  ...
  for (unsigned int row = 0; row < Map::IslandMap.size(); ++row) {
    for (unsigned int col = 0; col < Map::IslandMap[row].size(); ++col) {
      ...
      triangle[1].texCoords =
          sf::Vector2f(static_cast<float>(rect.position.x + rect.size.x), static_cast<float>(rect.position.y));
      triangle[2].texCoords = sf::Vector2f(static_cast<float>(rect.position.x + rect.size.x),
      ...
      triangle[4].texCoords = sf::Vector2f(static_cast<float>(rect.position.x + rect.size.x),
                                           static_cast<float>(rect.position.y + rect.size.y));
      triangle[5].texCoords =
          sf::Vector2f(static_cast<float>(rect.position.x), static_cast<float>(rect.position.y + rect.size.y));
    }
  }

  m_eventDispatcher.subscribe<DrawEvent>(this, [this](DrawEvent& event) { draw(event.getWindow()); }, RenderLayer::MAP);
}
...
void Rift::draw(sf::RenderWindow& window) {
  ...
  sf::FloatRect viewBounds(
      ...
      {currentView.getSize().x, currentView.getSize().y});
  ...
  triangle[0].texCoords =
      sf::Vector2f(static_cast<float>(waterRect.position.x), static_cast<float>(waterRect.position.y));
  triangle[1].texCoords = sf::Vector2f(static_cast<float>(waterRect.position.x + waterRect.size.x),
                                       static_cast<float>(waterRect.position.y));
  triangle[2].texCoords = sf::Vector2f(static_cast<float>(waterRect.position.x + waterRect.size.x),
                                       static_cast<float>(waterRect.position.y + waterRect.size.y));

  // texture coordinates for second triangle
  triangle[3].texCoords =
      sf::Vector2f(static_cast<float>(waterRect.position.x), static_cast<float>(waterRect.position.y));
  triangle[4].texCoords = sf::Vector2f(static_cast<float>(waterRect.position.x + waterRect.size.x),
                                       static_cast<float>(waterRect.position.y + waterRect.size.y));
  triangle[5].texCoords = sf::Vector2f(static_cast<float>(waterRect.position.x),
                                       static_cast<float>(waterRect.position.y + waterRect.size.y));
